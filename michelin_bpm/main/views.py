...
class FixMistakesView(UpdateProcessView):
    ...
    def get_form(self, form_class=None):
        ...
        if self.request.method == 'GET' and form.instance:
            corrections = form.instance.get_corrected_fields()
    ...
    def form_valid(self, form, **kwargs):
        """ Деавтивируем объект Корректировки, если произошло изменение блокирующих полей """
        super(FlowMixin, self).form_valid(form, **kwargs)
        # TODO MBPM-3
        # Сделать изменение одним запросом, если это возможно
        for task in form.instance.task_set.filter(correction__is_active=True):
